...
export default class Purge extends Command {
	...
	public async run (bot:Main, message:Discord.Message, args:string[], calledName:string):Promise<any> {
	    ...
	    const numToPurge: number = parseInt(args[0]);
	    ...
	    const numOfMessagesFinal: number = messagesInChannel.size;
	    ...
	    const actionChannel = await bot.channels.cache.get(bot.config.homeGuild.homeGuildActionChannel) as TextChannel;
