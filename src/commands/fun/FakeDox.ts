...
export default class FakeDox extends Command {
    ...
    public async run (botClient:Main, message:Discord.Message, args:string[], calledName:string):Promise<any> {
        ...
        if ((mentioned.id === message.author.id) || (mentioned.id === botClient.user.id)) {
            ...
            const badUsrWarn:Discord.Message = (await message.reply(`:x: HEY! Sorry, but you can't dox that person.`));
        ...
        const doxAlert:Discord.Message = (await message.channel.send(`:hourglass: Doxxing <@${mentioned.id}>. Please wait...`));
        ...
        const doxEmbed:Discord.MessageEmbed = (new Discord.MessageEmbed()
