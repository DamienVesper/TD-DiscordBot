...
export default abstract class Command implements ICommand, ICommandField {
    ...
    public abstract run<T>(botClient:Main, message:Discord.Message, args:string[], calledName:string):Promise<T>;
    ...
    public assertArgCount (argCount:number, message:Discord.Message):void {
        ...
        if (!(MathUtil.numberInRange(argCount, minArgs, maxArgs))) {
            ...
            const argLimPhrases:string[] = [`Not enough`, `minimum`, minArgs.toString()];
    ...
    private hasReqPerms (msgSender:Discord.GuildMember, permsRequired:string[]):boolean {
        ...
        for (const permIndex in permsRequired) {
            ...
            const curPerm:string = (permsRequired[permIndex]);
            ...
            if (Discord.Permissions.FLAGS[curPerm]) {
                ...
                const hasPermission:boolean = (msgSender.hasPermission(Discord.Permissions.FLAGS[curPerm]));
    ...
    public getMissingPerms (msgSender:Discord.GuildMember):string[] {
        ...
        for (const permIndex in this.permsRequired) {
            // Get the permission at the current index
            const curPerm:string = (this.permsRequired[permIndex]);
    ...
    public async queueArrayDelete (messageQueue:Pair<Discord.Message|Discord.Message[], number>[]):Promise<void> {
        ...
        for (const i in messageQueue) {
            ...
            const message:Discord.Message|Discord.Message[] = (messageQueue[i].getElemOne());
            const millis:number = (messageQueue[i].getElemTwo());
