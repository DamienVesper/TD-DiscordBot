...
const ArrayUtil_1 = __importDefault(require("../../../util/ArrayUtil"));
...
const InterpreterException_1 = __importDefault(require("./InterpreterException"));
...
class Interpreter {
     ...
     */
    constructor(botClient, cmdConsole) {
     ...
     */
    getRegisteredCommand(comName) {
        // Run a for loop over all of the registered commands
        for (const i in this.registeredCommands) {
            // Get the current registered command
            const curCom = this.registeredCommands[i];
            // Check if the input command name equals the name or alias of a registered command
            if ((comName.toLowerCase() === curCom.name.toLowerCase()) || (StringUtil_1.default.equalsAny(comName, curCom.aliases, true))) {
    ...
    register(comInst) {
        ...
        for (const i in this.registeredCommands) {
            ...
            const regCom = this.registeredCommands[i];
            ...
            if (comInst.name.toLowerCase() === regCom.name.toLowerCase()) {
                ...
                throw new InterpreterException_1.default(`The same command can't be registered twice.`);
            }
            // Loop over the aliases of the command to register
            for (const i in comInst.aliases) {
                // Check if the input command object's current alias is the same as one in the current existing command
                if (StringUtil_1.default.equalsAny(comInst.aliases[i], regCom.aliases, true)) {
                    // Throw an InterpreterException because the same command alias can't be registered twice
                    throw new InterpreterException_1.default(`The same command alias can't be registered twice.`);
                }
            }
        }
        // Check if the input command object has duplicated aliases or an alias that is the same as the name of the command
        if ((comInst.aliases.length !== new Set(comInst.aliases).size) || (StringUtil_1.default.equalsAny(comInst.name, comInst.aliases, true))) {
            // Throw an InterpreterException because a command can't have duplicated aliases or the name of the command as an alias
            throw new InterpreterException_1.default(`Command aliases can't contain duplicate entries or the name of the command.`);
        }
    ...
    async execute(message, args) {
        ...
        await AsyncUtil_1.default.asyncForEach(this.registeredCommands, async (exsCom) => {
            ...
            if ((args[0].toLowerCase() === exsCom.name.toLowerCase()) || (StringUtil_1.default.equalsAny(args[0], exsCom.aliases, true))) {
        ...
        if (execCom && execCom !== null) {
            ...
            if ((execCom.whitelistedGuilds.length >= 1) && (!StringUtil_1.default.equalsAny(message.guild.id, execCom.whitelistedGuilds, false))) {
                // Warn that the guild is not whitelisted
                await message.reply(`:no_entry_sign: This command isn't allowed to be used in this server. Please check with my managers to see which guild(s) this command is allowed in.`);
            ...
            if (!execCom.antiSpamAllowMsg(message.member) && !((this.botClient.config.botAdmins.includes(message.author.id)) || (message.author.id === this.botClient.config.ownerID))) {
                ...
                await message.reply(`:alarm_clock: Please wait ${Math.floor(execCom.spamTimeout / 1000)} second(s) before using this command again!`);
            ...
            if (execCom.trustedOnly || execCom.ownerOnly) {
                ...
                if (!hasPerm) {
                    ...
                    let missingPos = `a trusted bot admin`;
            ...
            if (!(execCom.hasPermission(message.member))) {
                ...
                const permDeniedPrefixes = [`I cannot fufill this request.`, `HEY!`, `Hold on!`];
                // Get the permissions that the user lacked
                const missingPerms = (await execCom.getMissingPerms(message.member));
                // Compose the response message and reply to the command sender
                await message.reply(`:lock: ${ArrayUtil_1.default.getRandomArrElem(permDeniedPrefixes)} Sorry, but you lack the proper permissions to run this command. You're missing the following permission(s): \`${ArrayUtil_1.default.toList((execCom.getMissingPerms(message.member)), `, `)}\`.`);
                // Resolve the promise, as the user lacked the permission to run the command, but return true because everything else went smoothly
                return Promise.resolve(true);
            }
            ...
            await execCom.run(this.botClient, message, (args.slice(1, (args.length))), args[0]).then(async (cmdResponse) => {
                ...
                if (error.name === CommandArgException_1.default.name) {
                    // Set the argument info string
                    let argInfo = (`between ${execCom.minArgs} and ${execCom.maxArgs}`);
                    ...
                    // Check if at least one of the limits was overridden (minimum args can't be lower than 0 in any case, so its override number is 0 instead of -1)
                        ...
                        const capType = ((execCom.maxArgs < 0) ? `at least` : `no more than`);
                        // Select which of the argument caps to use
                        const capNumber = ((execCom.maxArgs < 0) ? execCom.minArgs : execCom.maxArgs);
